...
Only the commissioner or system owner changes official game data through `Commissioner Dashboard` or `Corrections Queue`. Coaches submit routine finals and correction requests through their team portal.

## Decision table

| Situation | Commissioner action | Public effect |
| --- | --- | --- |
| Postponed, new date unknown | On `Commissioner Dashboard`, set `Status` to `Postponed`; keep the Game ID; add a brief public note | Game remains listed but is excluded from upcoming/live logic |
...
| Published score is wrong | Coach submits `Correction Request`; commissioner reviews `Corrections Queue` and sets it to `Approved` or `Rejected` | Approved results, standings, and seeding recalculate; the request remains auditable |
...
1. Confirm the row in `Commissioner Dashboard` is internally consistent.
...
5. Review `Score Audit` or `Corrections Queue` and preserve supporting league communication.
